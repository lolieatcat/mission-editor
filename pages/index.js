    ...
    return (
      <div className={styles.normal}>
        ...
        <div className={styles.panel} style={{marginTop:"20px"}}>
          ...
          <p>8）結果中的描述是過場文本，可以描述作出選擇後發生的事項。</p>
          <p>9）描述内容直接在小方框中輸入，必須有另一個結果在描述後出現；</p>
          <p>10）同一个选项的结果是会从结果0开始顺序执行的；</p>
          <p>11）如果结果选结束，或者没有结果，那么副本就默认结束了；</p>
          <p>12）其它的是属性的修改，可输入正值或负值，对于《序列战争》不建议数值过大；</p>
          <p>13) 副本总数值范围建议为：金镑[-500~+500], 经验[-200~+200], 灵性[-50~+50]</p>
          <p>14) 副本写好后，把保存的.json文件发给机器人管理员就可以了；</p>
          <h3>再说几句闲话</h3>
